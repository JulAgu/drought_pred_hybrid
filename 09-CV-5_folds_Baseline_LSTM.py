...
for fold, (train_index, test_index) in enumerate(kf.split(dataset)):
    ...
    X_tabular_train = []
    for _, X_tab_train, _ in train_loader:
    ...
    for i in range(epochs):
        ...
        torch.save(model.state_dict(), f"models/lstm_baseline_{fold+1}.pt")
    ...
    with torch.no_grad():
        if k == len(train_loader) - 1 or k == (len(train_loader) - 1) // 2:
            ...
            raw_labels = []
            raw_preds = []
            for inp, stat, lab in test_loader:
            ...
            results.to_csv(f"results/lstm_baseline_{fold+1}.csv", index=False)
