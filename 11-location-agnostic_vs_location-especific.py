...
EXPE_NAME = "location-agnostic_vs_location-especific"
...
# list_cat = [len(np.unique(X_tabular_cat_train[:,i])) + 1 for i in range(X_tabular_cat_train.shape[1])] # TODO: Don't forget to do this at each iteration

states_ranges = {"Iowa" : (19001, 19197),
                 "Montana" : (5001, 5149),
                 "Oklahoma" : (4001, 4027),
}
...
with open("data/data.pkl", "rb") as f:
    data = pickle.load(f)
or_X_tabular_train = data["X_tabular_train"]
or_X_time_train = data["X_time_train"]
or_y_target_train = data["y_target_train"]
or_X_tabular_validation = data["X_tabular_validation"]
or_X_time_valid = data["X_time_valid"]
or_y_target_valid = data["y_target_valid"]
or_valid_fips = data["valid_fips"]
or_X_tabular_test = data["X_tabular_test"]
or_X_time_test = data["X_time_test"]
or_y_target_test = data["y_target_test"]
or_test_fips = data["test_fips"]

def location_agnostic_vs_location_especific(state, start, end):
    X_tabular_train = or_X_tabular_train.copy()
    X_time_train = or_X_time_train.copy()
    y_target_train = or_y_target_train.copy()
    X_tabular_validation = or_X_tabular_validation.copy()
    X_time_valid = or_X_time_valid.copy()
    y_target_valid = or_y_target_valid.copy()
    valid_fips = or_valid_fips.copy()
    X_tabular_test = or_X_tabular_test.copy()
    X_time_test = or_X_time_test.copy()
    y_target_test = or_y_target_test.copy()
    test_fips = or_test_fips.copy()

    train_index = np.where((X_tabular_train[:,21] >= start) & (X_tabular_train[:,21] <= end))
    X_tabular_train = X_tabular_train[train_index]
    X_time_train = X_time_train[train_index]
    y_target_train = y_target_train[train_index]

    valid_index = np.where((X_tabular_validation[:,21] >= start) & (X_tabular_validation[:,21] <= end))
    X_tabular_validation = X_tabular_validation[valid_index]
    X_time_valid = X_time_valid[valid_index]
    y_target_valid = y_target_valid[valid_index]

    test_index = np.where((X_tabular_test[:,21] >= start) & (X_tabular_test[:,21] <= end))
    X_tabular_test = X_tabular_test[test_index]
    X_time_test = X_time_test[test_index]
    y_target_test = y_target_test[test_index]
...
if __name__ == "__main__":
    ...
    for state, (start, end) in states_ranges.items():
        loss_val = location_agnostic_vs_location_especific(state, start, end)
        print(f"Val loss for {state} is {loss_val}")